...
def test_sub():
    ...
    assert res == "아리랑 아리랑 아라리요. 아리랑 고개로 넘어갈 거예요. 나를 버리고 가시는 임은 십 리도 못가서 발병 날 거예요."
...
### KRE_Match Method Calls

def test_kre_match_search():
...
def test_kre_match_fullmatch():
    pass

def test_kre_match_sub():
    m = kre.compile('ㄴ다')
    res = m.sub('ㄹ 거예요', arirang)
    assert res == "아리랑 아리랑 아라리요. 아리랑 고개로 넘어갈 거예요. 나를 버리고 가시는 임은 십 리도 못가서 발병 날 거예요."

def test_kre_match_subn():
    nonsense = '할ㄱ으하느늘근ㅡ'
...
def test_kre_match_split():
    pass

def test_kre_match_findall():
    m = kre.compile('ㅏㄹ')
